...
def Bagging(train_filename, valid_filename, test_filename):
  ...
  #Reading Train data
        ...
        for s in splitter:
            for d in max_depth:
                for f in max_features:
                    dtc = DecisionTreeClassifier(criterion= c, splitter= s, max_depth=d, max_features=f)
                    ...
                    print(" Criterion: ",c," splitter:",s," max_depth: ",d," max_features: ",f," Accuracy: ",acc)
                    if(acc>best_acc):
                        best_acc = acc
                        best_c = c
                        best_s = s
    ...
    dtc = DecisionTreeClassifier(criterion=best_c, splitter=best_s, max_depth=best_d, max_features=best_f)
    clf = BaggingClassifier(base_estimator=dtc)
    ...
    print("     best_criterion: ",best_c," best_splitter:",best_s," best_max_depth: ",best_d," best_max_features: ",best_f)
...
for filename in os.listdir("../all_data/"):
    if(filename.startswith('train')):
        ...
        Bagging(train_filename, valid_filename, test_filename)
        print(' ')
